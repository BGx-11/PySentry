...
Start the detection engine. It requires Administrator privileges to scan all processes effectively.

**Windows:**
```powershell
python pysentry.py
(Run your terminal as Administrator for best results)
...
To verify that PySentry is working, use the included Safe Simulator. This script mimics the behavior of a malware beacon (connecting to a public DNS) without doing anything harmful.
...
Check your PySentry window. You should see a 🔴 HIGH RISK alert for the simulator process.
...
SAFE_PATHS = [
    r"C:\My\Custom\App\Folder",
    "TrustedApp.exe"
]
SCRIPT_ENGINES: Add other interpreters you want to monitor (e.g., ruby, perl, java).
